...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async () => {
				const items: Array<
					vscode.QuickPickItem & { commandId: string }
				> = [
					{
			...
			async () => {
				...
				if (panel) {
					...
					disposables.push(
						qp.onDidAccept(async () => {
							...
							if (sel.isGeneration) {
								...
								const pokemonInGeneration =
									getPokemonByGeneration(
										sel.gen as PokemonGeneration,
									);
								const pokemonOptions = pokemonInGeneration.map(
									(type) => ({
					...
					if (!selectedPokemonType) {
						console.log(
							'Cancelled Spawning Pokemon - No Selection',
						);
						return;
					}
					...
					if (!selectedPokemonType) {
						console.log(
							'Cancelled Spawning Pokemon - No Pokémon Selected',
						);
						return;
					}
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration(
			(e: vscode.ConfigurationChangeEvent): void => {
				...
				if (
					e.affectsConfiguration('vscode-pokemon.throwBallWithMouse')
				) {
	...
	public static createOrShow(
		...
		if (PokemonPanel.currentPanel) {
			if (
				...
				pokemonGeneration ===
					PokemonPanel.currentPanel.pokemonGeneration()
			) {
