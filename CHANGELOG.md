...
- Add evolution and generation filtering to auto-spawn
- Add one-time migration for users upgrading from pre-release versions with minute-based intervals
- Update `autoSpawn.behavior` enum and default in configuration schema
- Enforce unique values in `autoSpawn.generations` config
